...
本插件依赖一个独立的后处理模型，用于执行分段、清洗、审查和复审。
...
- `force_local_fallback_reason`：命中后直接回退本地分段的特殊拒绝原因
...
6. 若后处理模型返回约定的特殊拒绝原因，插件会忽略该次模型结果并直接切换到本地 fallback 分段，避免分段失效。
7. 本地 fallback 默认最多保留 12 段；若开启“限制段数”，则优先服从 `min_segments / max_segments`。
8. 若开启 `debug_log`，日志中会记录较长的提示词和模型输出，调试完成后建议关闭。
